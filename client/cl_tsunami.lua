...
-- 3 Minutes Reminder
RegisterNetEvent('strez:client:SendMailTsunamiAuto3', function(text)
    TriggerEvent('InteractSound_CL:PlayOnOne', 'Alert', 0.1)
    Citizen.Wait(10000) -- Use Citizen.Wait instead of Wait

    local message = 'Emergency Weather Update: Power outage in the city. A tsunami is imminent. Coastal residents, move to higher ground immediately. Follow official instructions and avoid beaches and low-lying areas.'
    ...
    Citizen.Wait(5000) -- Use Citizen.Wait instead of Wait

    -- Assuming these are valid function calls, otherwise replace with actual function calls
    TriggerServerEvent('qb-weathersync:server:toggleBlackout')
	Wait(1000) -- Use Citizen.Wait instead of Wait
	TriggerServerEvent('qb-weathersync:server:toggleBlackout')
	Wait(500) -- Use Citizen.Wait instead of Wait
	TriggerServerEvent('qb-weathersync:server:toggleBlackout')
